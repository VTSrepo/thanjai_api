...
class ProductDao extends BaseDao {
        ...
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                //  c.ref_type="PATTYP" and c.ref_code=b.insurance_type_id`;
                 ...
                 WHERE s.product_id='${product_id}' and active_flag='Y'  ORDER BY updated_date DESC `;
    ...
    getProductMasterByOrgId(connection, org_id, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                if(query.filter.hasOwnProperty('bu_id') && query.filter.bu_id!=null){
                        input += ` AND p.bu_id = '${query.filter.bu_id}' `;
                }
                ...
                WHERE ${input}`;
          //      console.log("getProductMasterByORg", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                   // console.log('Sorry, Product Data Not Found!.', queryres);
                    var error_code = { status: 404, code: 4001, message: "Sorry, Product Data Not Found!.", developerMessage: "Sorry, Product Data Not Found!." };
    ...
    getEmployeeProducts(connection, org_id, query) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                if(query.filter.hasOwnProperty('emp_id') && query.filter.emp_id!=null){
                        ...
                        input += ` AND d.emp_id = '${query.filter.emp_id}' `;
                    
                }

              
                var custQuery = `SELECT p.*,e.emp_name,m.product_name  FROM ${process.env.WRITE_DB_DATABASE}.production_data p 
                ...
                WHERE  ${input}`;
                debug("getEmployeeProducts", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    debug('Sorry, Product Data Not Found!.');
                    var error_code = { status: 404, code: 4001, message: "Sorry, Product Data Not Found!.", developerMessage: "Sorry, Product Data Not Found!." };
                    return resolve(error_code)
                }
                else{
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('getEmployeeProducts Error :', error)
                return reject(err_code);
            }
        })
    }
