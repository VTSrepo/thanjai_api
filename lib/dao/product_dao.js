        ...
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                //  c.ref_type="PATTYP" and c.ref_code=b.insurance_type_id`;
                 ...
                 var custQuery = `SELECT s.*,DATE_FORMAT(eff_from,'%Y-%m-%d') as eff_from FROM ${process.env.WRITE_DB_DATABASE}.prod_selling_price_master s WHERE s.product_id='${product_id}' `;
