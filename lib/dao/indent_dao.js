...
class IndentDao extends BaseDao {
    ...
    GetIndentReport(connection,from_date,to_date,status) {
        return new Promise(async(resolve, reject)=> {
            ...
            if(status!=null && status.trim().length>0){
                custQuery = `SELECT a.branch_id,c.branch_name,DATE_FORMAT(a.indent_date,'%Y-%m-%d')  as indent_date, a.indent_number,a.status,a.delivery_by_datetime,a.self_customer,
       a.kitchen_id,d.branch_name kitchen_name,b.item_code,e.product_name, b.qty_ordered,b.qty_agreed_kitchen,
       b.qty_received,b.item_cost,b.net_value item_tot_cost,a.value indent_value, 
       b.kitchen_remarks,b.discrepancy_notes,b.damage_notes 
       from ${process.env.WRITE_DB_DATABASE}.indent_header a, ${process.env.WRITE_DB_DATABASE}.indent_detail b,
			${process.env.WRITE_DB_DATABASE}.branch_master c, ${process.env.WRITE_DB_DATABASE}.branch_master d,
            ${process.env.WRITE_DB_DATABASE}.product_master e
            where 
            a.indent_number=b.indent_number and
            a.branch_id=c.branch_id and
            a.kitchen_id=d.branch_id and
            b.item_code=e.product_id and
            a.status='${status}' AND a.indent_date between '${from_date}' and '${to_date}' ORDER BY a.indent_date,a.indent_number`;
            }else{
                custQuery = `SELECT a.branch_id,c.branch_name,DATE_FORMAT(a.indent_date,'%Y-%m-%d')  as indent_date, a.indent_number,a.status,a.delivery_by_datetime,a.self_customer,
       a.kitchen_id,d.branch_name kitchen_name,b.item_code,e.product_name, b.qty_ordered,b.qty_agreed_kitchen,
       b.qty_received,b.item_cost,b.net_value item_tot_cost,a.value indent_value, 
       ...
       from ${process.env.WRITE_DB_DATABASE}.indent_header a, ${process.env.WRITE_DB_DATABASE}.indent_detail b,
            ...
            a.kitchen_id=d.branch_id and
            b.item_code=e.product_id and
            a.indent_date between '${from_date}' and '${to_date}' ORDER BY a.indent_date,a.indent_number`;
            }
               

                //console.log("GetIndentReport", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    //console.log('Sorry, INTEND Not Found!.', queryres);
    ...
    fetchIndentNo(connection, branch_id, seq_type) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
    ...
    getIndentNumber(connection,org_id, indent_number) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
               ...
               var custQuery = `SELECT  d.* , DATE_FORMAT(d.indent_date,'%Y-%m-%d')  as indent_date, 
               (SELECT branch_name FROM ${process.env.WRITE_DB_DATABASE}.branch_master where  branch_id=d.branch_id) as branch_name,
               (SELECT branch_name FROM ${process.env.WRITE_DB_DATABASE}.branch_master where  branch_id=d.kitchen_id) as kitchen_name,
                DATE_FORMAT(d.created_date,'%Y-%m-%d')  as created_date,DATE_FORMAT(d.updated_date,'%Y-%m-%d')  as updated_date 
                FROM ${process.env.WRITE_DB_DATABASE}.indent_header d 
               WHERE   d.org_id='${org_id}' and d.indent_number='${indent_number}'`;
             //   debug("getPoNumber", custQuery)
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                debug('get indent detail error :', error)
                return reject(err_code);
            }
        })
    }

    GetIndentDetailList(connection,  indent_number) {
        return new Promise(async(resolve, reject)=> {
            try{
                var returnres=null;
                var custQuery;
               ...
               custQuery = `SELECT  d.* , DATE_FORMAT(d.indent_date,'%Y-%m-%d')  as indent_date, p.product_name as item_name,p.uom,
                DATE_FORMAT(d.created_date,'%Y-%m-%d')  as created_date,DATE_FORMAT(d.updated_date,'%Y-%m-%d')  as updated_date   FROM ${process.env.WRITE_DB_DATABASE}.indent_detail d 
                LEFT JOIN ${process.env.WRITE_DB_DATABASE}.product_master p on d.item_code=p.product_id WHERE d.indent_number='${indent_number}'`;
    ...
    GetIndentDetail(connection, org_id,branch_id,kitchen_id, indent_number,item_code) {
        return new Promise(async(resolve, reject)=> {
            try{
                var custQuery;
                if(connection == null) {
               ...
               custQuery = `SELECT  *   FROM ${process.env.WRITE_DB_DATABASE}.indent_detail d 
                 WHERE d.indent_number='${indent_number}' AND d.org_id='${org_id}'  AND d.branch_id='${branch_id}' 
                 AND d.kitchen_id='${kitchen_id}'  AND d.item_code='${item_code}'  `;

                //console.log("GetIndentDetail", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    //console.log('Sorry, Po Not Found!.', queryres);
                    var error_code = { status: 404, code: 4001, message: "Sorry, Data Not Found!.", developerMessage: "Sorry, Data Not Found!." };
    ...
    GetIndentListByAll(connection, org_id,query) {
        return new Promise(async(resolve, reject)=> {
            try{
                ...
                let input= `d.org_id='${org_id}'`;
                var flag="N"
                if(query.filter.hasOwnProperty('status') && query.filter.status!=null){
                    input += ` AND d.status = '${query.filter.status}' `;
                  
                }
                if(query.filter.hasOwnProperty('branch_id') && query.filter.branch_id!=null){       
                        input += ` AND d.branch_id = '${query.filter.branch_id}' `;
                }

                if(query.filter.hasOwnProperty('kitchen_id') && query.filter.kitchen_id!=null){     
                        input += ` AND d.kitchen_id = '${query.filter.kitchen_id}' `;
                }

                if(query.filter.hasOwnProperty('indent_number') && query.filter.indent_number!=null){
                         input += ` AND d.indent_number = '${query.filter.indent_number}' `;
                }
                custQuery = `SELECT d.* , DATE_FORMAT(d.indent_date,'%Y-%m-%d')  as indent_date, 
                (SELECT branch_name FROM ${process.env.WRITE_DB_DATABASE}.branch_master where  branch_id=d.branch_id) as branch_name,
               (SELECT branch_name FROM ${process.env.WRITE_DB_DATABASE}.branch_master where  branch_id=d.kitchen_id) as kitchen_name,
                DATE_FORMAT(d.created_date,'%Y-%m-%d')  as created_date,DATE_FORMAT(d.updated_date,'%Y-%m-%d')  as updated_date FROM ${process.env.WRITE_DB_DATABASE}.indent_header d 
                 WHERE ${input}`;
                //console.log("GetIndentListsByOrgId", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    //console.log('Sorry, Po Not Found!.', queryres);
                    var error_code = { status: 404, code: 4001, message: "Sorry, Data Not Found!.", developerMessage: "Sorry, Data Not Found!." };
                ...
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                //console.log('GetIndentListByStatus error :', error)
                return reject(err_code);
            }
        })
    }
    ...
    updateIndentDetail(connection, set_indent_detail,org_id,branch_id,indent_number,kitchen_id,item_code) {
        return new Promise(async(resolve, reject) => {
            try {
                if(connection == null ) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!.", developerMessage:"DB Connection Failed!." };
                ...
                await connection.query(`UPDATE ${process.env.WRITE_DB_DATABASE}.indent_detail SET ? WHERE org_id='${org_id}' 
                    AND branch_id='${branch_id}' AND indent_number='${indent_number}' AND kitchen_id='${kitchen_id}' AND item_code='${item_code}' `, set_indent_detail);
                //console.log('COMMIT at Update detail');
                return resolve(set_indent_detail);
            }
            catch (err) {
                var err_code = { status: 500, code: 5001, message: "Sorry, Internal Server Error!.", developerMessage:"Sorry, Internal Server Error!." };
                //console.log("Update Indent detail error :", err);
                return reject(err_code);
            }
        })
    }
    ...
    GetIndentListByOrgId(connection, org_id) {
        return new Promise(async(resolve, reject)=> {
            try{
                var custQuery, inv_status;
                if(connection == null) {
    ...
    GetIndentListByStatus(connection, org_id,status) {
        return new Promise(async(resolve, reject)=> {
            try{
                var custQuery, inv_status;
                if(connection == null) {
                ...
                custQuery = `SELECT d.* , DATE_FORMAT(d.indent_date,'%Y-%m-%d')  as indent_date, b.branch_name,k.branch_name as kitchen_name,
                DATE_FORMAT(d.created_date,'%Y-%m-%d')  as created_date,DATE_FORMAT(d.updated_date,'%Y-%m-%d')  as updated_date FROM ${process.env.WRITE_DB_DATABASE}.indent_header d 
                INNER JOIN ${process.env.WRITE_DB_DATABASE}.branch_master b ON d.branch_id=b.branch_id 
                INNER JOIN branch_master k ON k.branch_id=d.kitchen_id WHERE 
                 d.org_id='${org_id}' and d.status='${status}'`;
                //console.log("GetIndentListsByOrgId", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    //console.log('Sorry, Po Not Found!.', queryres);
