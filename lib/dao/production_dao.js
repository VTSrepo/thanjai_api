...
class ProductionDao extends BaseDao {
    ...
    GetProduction(connection, org_id, branch_id, emp_id, product_id, production_date, start_time, end_time) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
    ...
    ferchEmployeeId(connection, branch_id, seq_type) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
        ...
        return new Promise(async(resolve, reject) => {
            try {
                ...
                await connection.query(`UPDATE ${process.env.WRITE_DB_DATABASE}.emp_master SET ? 
                    WHERE org_id='${org_id}' and branch_id='${branch_id}' and emp_id='${emp_id}' `, set_employee_data);
