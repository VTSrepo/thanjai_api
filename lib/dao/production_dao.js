...
class ProductionDao extends BaseDao {
    ...
    createProduction(connection, product_data) {
        return new Promise(async(resolve, reject) => {
            try {
                ...
                //console.log('COMMIT at createProduction', product_data);
    ...
    fetchSeqEmpId(connection, branch_id, seq_type,org_id) {
        return new Promise(async(resolve, reject)=> {
            try{
                if(connection == null) {
                    var err_code = { status: 500, code: 5001, message: "DB Connection Failed!", developerMessage:"DB Connection Failed!"};
                ...
                var custQuery = `SELECT concat(concat('${org_id}',seq_type),LPAD(last_seq_no+1,3,'0')) as emp_id ,last_seq_no+1 as last_seq_no 
                FROM ${process.env.WRITE_DB_DATABASE}.vts_seq_generator WHERE seq_type='${seq_type}'`;
              //  console.log("fetchEmployeeId", custQuery)
                let queryres = await connection.query(custQuery);
                if(queryres.length == 0) {
                    ...
                    var newpatientquery = `SELECT concat(concat('${org_id}',seq_type),LPAD(0,3,'0')) as emp_id ,last_seq_no as last_seq_no 
                    FROM ${process.env.WRITE_DB_DATABASE}.vts_seq_generator WHERE seq_type='${seq_type}'`;
