        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if(query.filter.hasOwnProperty('branch_id') && query.filter.branch_id!=null){
                        input += ` AND branch_id = '${query.filter.branch_id}' `;
                }

                if(query.filter.hasOwnProperty('user_id') && query.filter.user_id!=null){
                    input += ` AND user_id = '${query.filter.user_id}' `;
               }
                //console.log('in dao');
                var custQuery = `SELECT * FROM ${process.env.WRITE_DB_DATABASE}.user_master where ${input}`;
                console.log(custQuery);
