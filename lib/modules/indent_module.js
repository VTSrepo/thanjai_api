...
class IndentModule {
   ...
   indentSubmission(data,  query) {
    ...
    GetIndentListsByBranchId(branch_id,kitchen_id, query) {
        return new Promise(async(resolve, reject) => {
            var indentDao = new IndentDao();
            var connection = null;
            var today = new Date();
            ...
            try {
                ...
                if(query.filter.hasOwnProperty('indent_status')) {
           
                    get_indent = await indentDao.GetIndentListsByStatus(connection, branch_id,kitchen_id,query.filter.indent_status, query, strPagination);
                }else{
                    get_indent = await indentDao.GetIndentListsByBranchId(connection, branch_id,kitchen_id, query, strPagination);
                }
    ...
    GetIndentDetail(indent_number, query) {
        return new Promise(async(resolve, reject) => {
            var indentDao = new IndentDao();
            var connection = null;
            var get_indent;
            try {
                connection = await indentDao.getReadConnection();
                get_indent = await indentDao.getIndentNumber(connection, indent_number);
                if(get_indent.hasOwnProperty('status') && get_indent.status == 404) {
                    if (connection) {
                        await indentDao.releaseReadConnection(connection);
                    }
    ...
    return new Promise(async(resolve, reject) => {
        try {
            ...
            if(data.hasOwnProperty('delivery_by_datetime')) {
                delivery_by_datetime= moment(data.delivery_by_datetime).format("YYYY-MM-DD HH:mm:ss");
            }
            var indent_data = {
                org_id: data.org_id, 
