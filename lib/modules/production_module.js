...
function categories_data_to_schema_proudction_data_to_create(connection, data,datetime){
    return new Promise(async(resolve, reject) => {
        try {
            ...
            if(data.hasOwnProperty('production_date')) {
                production_date= moment(data.production_date).utc().format("YYYY-MM-DD HH:mm:ss");
            }
            var start_time=null;
            if(data.hasOwnProperty('start_time')) {
                start_time= moment(data.start_time).utc().format("YYYY-MM-DD HH:mm:ss");
            }
            var end_time=null;
            if(data.hasOwnProperty('end_time')) {
                end_time= moment(data.end_time).utc().format("YYYY-MM-DD HH:mm:ss");
            }
