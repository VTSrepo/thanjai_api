    ...
    GetCategoryList(query) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                //set_pat_adv_payment_data = await categories_data_to_schema_advance_payment_data_to_create(read_connection, data, date);
                var get_categories = await masterDao.getCategoryList(read_connection,query);
                if(get_categories.hasOwnProperty('status') && get_categories.status == 404) {
                    ...
                    var page_size = get_categories.length//query.skip ? query.skip : total_size;
                    var result_size = get_categories.length//strLimit;
                    console.log("Totalsize :", total_size);
                    var summary = {
                        filteredsize: page_size, resultsize: result_size, totalsize: total_size
                    };
    ...
    GetBranchList(query,org_id) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                //set_pat_adv_payment_data = await categories_data_to_schema_advance_payment_data_to_create(read_connection, data, date);
                var get_branches = await masterDao.getBranches(read_connection,org_id,query);
  ...
  CreateAccount(data,  query) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if(data.hasOwnProperty('category_code') && (data.category_code != null && data.category_code.length >3)) {
                var get_master_data = await masterDao.getCategoryDetail(read_connection, data.category_code);
                console.log(get_master_data);
                if(get_master_data.hasOwnProperty('status') && get_master_data.status == 404) {
                    
                    set_master_data = await categories_data_to_schema_category_to_create(read_connection, data, date);
                    console.log("get_master_data", set_master_data)
                    master_data = await masterDao.createCategory(read_connection, set_master_data);
                    if (read_connection) {
                        await masterDao.releaseReadConnection(read_connection);
                    }
                    var final_res={
                    ...
                    console.log("Update",get_master_data.category_code);
                    user_master = await categories_data_to_schema_category_to_update(data, get_master_data, date);
                    master_data = await masterDao.updateCategory(read_connection, user_master, data.category_code);
                    if (read_connection) {
                        await masterDao.releaseReadConnection(read_connection);
                    }
                    var final_res={
                ...
                set_master_data = await categories_data_to_schema_category_to_create(read_connection, data, date);
                console.log("set_doctor_data", set_master_data)
                master_data = await masterDao.createCategory(read_connection, set_master_data);
                if (read_connection) {
                    await masterDao.releaseReadConnection(read_connection);
                }
...
function generateCategoryId(connection, data, seq_type) {
    return new Promise(async(resolve, reject) => {
        var masterDao = new MasterDao();
        var category_detail, category_code;
        
        try{
            category_detail = await masterDao.fetchCategoryId(connection, seq_type);
            if(category_detail != null) {
                category_code = category_detail.category_code;
                return resolve(category_code);
            }
            else{
...
function categories_data_to_schema_category_to_create(connection, data,  date){
    return new Promise(async(resolve, reject) => {
        try {
            var category_code;
            var seq_type = `CAT`;
            category_code = await generateCategoryId(connection, data, seq_type)
            var category_data = {
                
                category_code: category_code, 
                category_name: data.category_name, 
                active: (data.active)?data.active:"A", 
...
function categories_data_to_schema_category_to_update(data, get_master_data, date){
    return new Promise(async(resolve, reject) => {
        try {
         
            var category_data = {
                category_name: data.hasOwnProperty('category_name')?data.category_name:get_master_data.category_name,
                active:  data.hasOwnProperty('active')?data.active:get_master_data.active,
                updated_by : (data.hasOwnProperty('user_id'))?data.user_id:null,  
                updated_date: date
